...
setGlobalOptions({maxInstances: 10, timeoutSeconds: 120});

const app = express();
app.use(express.json());
app.use(cors({origin: "https://temphumidity-in-blockchain.web.app"}));

console.log("Starting Cloud Function...");

const contractAddress = process.env.contract_address;
const prcUrl = process.env.prc_url;
...
if (!prcUrl) {
  console.error("Invalid PRC_URL");
  throw new Error("Invalid PRC_URL");
}
...
const abiFilePath = path.resolve(
  __dirname,
  "./artifacts/contracts/TempHumidity.sol/TempHumidity.json",
);
...
const provider = new ethers.JsonRpcProvider(prcUrl);
...
async function shouldUpload(temperature, humidity) {
  ...
  if (lastData.timestamp === 0) return true;
  if (timeSinceLast >= 3600) return true;

  const tempChanged = temperature > lastData.temperature ?
    temperature - lastData.temperature >= 2 :
    lastData.temperature - temperature >= 2;
  const humidityChanged = humidity > lastData.humidity ?
...
app.post("/api/storeData", async (req, res) => {
  ...
  if (!canUpload) {
    return res.json({
      success: false,
      error: "Conditions not met (time or change too small)",
    });
  }
