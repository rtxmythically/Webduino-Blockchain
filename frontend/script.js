const API_BASE_URL = window.location.hostname === "localhost" || window.location.hostname === "127.0.0.1"
  ? "http://localhost:3000"
  : "https://us-central1-temphumidity-in-blockchain.cloudfunctions.net/api";
...
async function updateBlockchainData() {
  try {
    const response = await fetch(`${API_BASE_URL}/api/latestRecord`);
    if (!response.ok) {
      throw new Error(`HTTP error! Status: ${response.status}`);
    }
    const data = await response.json();
...
async function sendToBackend(temp, hum) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/storeData`, {
      method: "POST",
    ...
    console.log("Store data response:", result);
    if (result.success) {
