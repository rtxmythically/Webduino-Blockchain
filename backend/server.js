...
async function shouldUpload(temperature, humidity) {
  if (await hasPendingTransactions()) {
    console.log("Pending transactions detected, skipping upload");
    return false;
  }
...
async function storeData(temperature, humidity) {
  try {
    ...
    console.log("Submitting transaction with nonce:", currentNonce);
    ...
    if (error.error?.message === "already known") {
      return { success: false, error: "Transaction already in mempool, please wait" };
    }
